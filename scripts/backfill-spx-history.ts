 ...
 * Uses ^gspc ticker; chunks by ~2 years. Longer history than SPY when available.
 *
...
async function main() {
  ...
  console.log("SPX history backfill: from", START_DATE, "to", toYYYYMMDD(end), "(ticker:", STOOQ_SPX_TICKER + ")");
  ...
  while (start < end) {
    ...
    try {
      const rows = await fetchStooqDateRangeForTicker(STOOQ_SPX_TICKER, SERIES_KEY, d1, d2, 3000);
      for (const p of rows) {
